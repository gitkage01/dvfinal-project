...
output$Boxplot__2_ <- renderPlotly({
  plot = dfA1() %>% ggplot() + 
    geom_boxplot(mapping = aes(x=retailer_country, y = revenue)) +
    ...
    ggtitle(paste("Revenue and Retailer Country", input$title__2_)) +
  ...
  if( ! is.na(input$plotWidth__2_) & ! is.na(input$plotHeight__2_))
    ggplotly(plot, tooltip = c("revenue"), session="knitr", width = input$plotWidth__2_, height = input$plotHeight__2_)
  else
