dfA <- eventReactive(c(input$selectCountry), { 
  project <- "https://data.world/diana-luong/s18-edv-project-3" 
  data.world::set_config(cfg_env("DW_API")) 
  paramQuery <- data.world::qry_sql(
    "   
    select revenue, retailer_country
...
dfA1 <- eventReactive(c(input$selectCountry, input$yDataMin__1_, input$yDataMax__1_), { 
  if( ! is.na(input$yDataMin__1_) & ! is.na(input$yDataMax__1_)) {
    dfA() %>% dplyr::filter(between(revenue, input$yDataMin__1_, input$yDataMax__1_))
  }
  else {
    dfA()
  }
