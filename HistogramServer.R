...
output$Histogram__1_ <- renderPlotly({
  plot = dfA1() %>% ggplot() + 
    geom_histogram(mapping = aes(x=revenue, colour=retailer_country, fill=retailer_country), binwidth = 20000) +
    ...
    ggtitle(paste("Revenue", input$title__1_)) +
  ...
  if( ! is.na(input$plotWidth__1_) & ! is.na(input$plotHeight__1_))
    ggplotly(plot, tooltip = c("revenue"), session="knitr", width = input$plotWidth__1_, height = input$plotHeight__1_)
  else
